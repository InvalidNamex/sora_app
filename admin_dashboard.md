...
* **Data Source:** `order_feedback` and `product_reviews`.
* **UI:** Summary delivery/product averages, delivery-rating filter, and expandable order cards containing customer delivery comments and per-product ratings/reviews.
* **Customer Flow:** A delivered order prompts its customer to rate delivery and each purchased product. The customer can later edit the feedback from Order Details.
